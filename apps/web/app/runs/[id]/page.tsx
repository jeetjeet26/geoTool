...
export default async function RunDetailPage({
  ...
  const backHref = clientId ? ({ pathname: '/runs', query: { clientId } } as any) : '/runs';
    ...
    <section className="flex flex-1 flex-col gap-10">
      ...
      <header className="page-header">
        ...
        <div className="flex flex-col items-start gap-3 text-sm text-slate-500 md:items-end">
          <span>{completionTimestamp ? `Completed ${formatDistanceToNow(completionTimestamp, { addSuffix: true })}` : 'Run still in progress'}</span>
